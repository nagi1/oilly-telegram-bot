...
const askOilChangeFrequency = async (/** @type {TelegramBot.Chat}*/ chat) => {
	const questionMarkup = `❓ *${i18n.__('oil_change_frequency')}* ❓`;
	await bot.sendMessage(chat.id, jse(questionMarkup), {
		parse_mode: 'MarkdownV2',
		reply_markup: {
			force_reply: true,
			selective: true,
			input_field_placeholder: i18n.__('oil_change_frequency_placeholder'),
		},
	});
	...
	await bot.sendMessage(chat.id, jse(`_${i18n.__('oil_change_frequency_explain')}_`), {
		parse_mode: 'MarkdownV2',
	});
...
const validationLogicForOilChangeFrequency = (answer) => {
	return isNumber(answer) && answer >= 0 && answer <= 365;
};
module.exports = { askOilChangeFrequency, validationLogicForOilChangeFrequency };
